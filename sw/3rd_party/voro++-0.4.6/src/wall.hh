...
/** \brief A class representing a cylindrical wall object with half-sphere on one end.
 *
 * This class represents a cylinder wall object with half-sphere on one end. */
struct wall_cylinder_sphere_end : public wall {
    public:
        /** Constructs a cylinder wall object.
         * \param[in] (xc_,yc_,zc_) a point on the axis of the
         *			    cylinder.
         ...
         * \param[in] w_id_ an ID number to associate with the wall for
         *		    neighbor tracking. */
        wall_cylinder_sphere_end(double xc_,double yc_,double zc_,double xa_,double ya_,double za_,double rc_,int w_id_=-99)
            : w_id(w_id_), xc(xc_), yc(yc_), zc(zc_), xa(xa_), ya(ya_), za(za_),
            asi(1/(xa_*xa_+ya_*ya_+za_*za_)), rc(rc_) {}
        bool point_inside(double x,double y,double z);
        template<class v_cell>
...
struct wall_user_callback : public wall {
    public:
