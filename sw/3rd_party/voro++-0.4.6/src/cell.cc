 ...
 * \param[in] (x,y,z) the position vector of the particle in the global
 *                    coordinate system. */
void voronoicell_base::output_vertices(std::vector<int> &v,double x,double y,double z,FILE *fp) {
    // create struct to hold a point
    // then make an array of these structs so they map to (only needs to hold enough for one face)
    // vertices of the Voronoi cell in the format (x,y,z), relative to the global coordinate system.
    // then step thru the vertices that make up each face (in vi) and output the (x,y,z) points for that vertices
    // next once we know the list of points for a face, arrange them in "6-value" & "9-value" csv output so
    // straight (6-value) and spline (9-value) lines can be drawn between them.
    //
    // for example:
    //    s=9
    //    t=(1,7,6,3) (1,13,8,0,7) (1,3,4,12,13) (2,8,13,12) (2,10,11,0,8) (2,12,4,5,10) (3,6,9,5,4) (5,9,11,10) (6,7,0,11,9)
    //    w=14
    //    P=(-3,-3,0) (-3,-2.00116,1.03375) (-2.04445,-3,1.29794) (-2.2084,-1.99708,0.723851) (-1.72753,-2.82154,1.09336) (-1.63103,-2.98712,1.03096) (-2.20875,-1.99708,0) (-3,-2.00116,0) (-3,-3,1.29461) (-1.63103,-2.98796,0) (-1.63103,-3,1.03095) (-1.63103,-3,0) (-2.04445,-2.94257,1.29797) (-3,-2.38834,1.2949)
    //
    // Output will look like:
    //    B=((-3,-2.00116,1.03375),(-3,-2.00116,0),(-2.20875,-1.99708,0),(-2.2084,-1.99708,0.723851))
    //    ((-3,-2.00116,1.03375),(-3,-2.38834,1.2949),(-3,-3,1.29461),(-3,-3,0),(-3,-2.00116,0))
    //    ((-3,-2.00116,1.03375),(-2.2084,-1.99708,0.723851),(-1.72753,-2.82154,1.09336),(-2.04445,-2.94257,1.29797),(-3,-2.38834,1.2949))
    //    ((-2.04445,-3,1.29794),(-3,-3,1.29461),(-3,-2.38834,1.2949),(-2.04445,-2.94257,1.29797))
    
    if ((p <= 0) || (v.size() <= 0)) {
        fprintf(stderr, "ERROR: voronoicell_base::output_vertices() no data points!\n");
        return;
    }
    ...
    aPoint lclpts[init_vertices];
    aPoint vpts[init_vertices];
    
	// collect up all points first
    int ix = 0;
    lclpts[ix++] = {x+*pts*0.5,y+pts[1]*0.5,z+pts[2]*0.5};
    for(double *ptsp=pts+3;ptsp<pts+3*p;ptsp+=3)
        lclpts[ix++] = {x+*ptsp*0.5,y+ptsp[1]*0.5,z+ptsp[2]*0.5};

	// now go thru all face vertices and ouptut each point xyz
    if(v.size()>0) {
        int k=0,l,face=0,idx;
        while((unsigned int) k<v.size()) {
            l=v[k++]; idx=0;
            fprintf(fp,"%d.%d\n",face++,l);
            if(l<=1) {
                if(l==1) { int d=v[k++]; vpts[idx++] = {lclpts[d].x,lclpts[d].y,lclpts[d].z}; }
                else fputs("# EMPTY",fp);   // TODO: should be assert?
            } else {
                int j=k+l;
                while(k<j) { int d=v[k++]; vpts[idx++] = {lclpts[d].x,lclpts[d].y,lclpts[d].z}; }
            }
            
#define OUTPUT_3VAL 0
#define OUTPUT_9VAL 1
            
#if OUTPUT_3VAL == 1
            for (int i=0; i<idx; i++)
                fprintf(fp,"%g,%g,%g\n",vpts[i].x,vpts[i].y,vpts[i].z);
            // complete face by re-printing first entry
            fprintf(fp,"%g,%g,%g\n",vpts[0].x,vpts[0].y,vpts[0].z);
            // and because 3val is making 3-point splines, repeat 2nd again
            fprintf(fp,"%g,%g,%g\n",vpts[1].x,vpts[1].y,vpts[1].z);
#elif OUTPUT_9VAL == 1
            aPoint p0=vpts[0],p1=vpts[1],p2=vpts[2];
	...
	// output file single number so next layer (fusion360 script) always draws last face
        fprintf(fp,"0.0\n");
    }
}
	...
	while(*fmp!=0) {
		if(*fmp=='%') {
			...
			switch(*fmp) {
                ...
                // sbh101316:
                // B # <- added 10/13 to output all spline & direct curves, combining 'num=%s[%t],num=%w[%P]'
                    // %s = number_of_faces()
                    // %t = face_vertices(vi); voro_print_face_vertices(vi,fp);
                    // %w = p
                    // %P = output_vertices(x,y,z,fp)
                case 'B':
